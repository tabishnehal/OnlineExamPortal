   ...
   if(!$ret) {
	...
	if(password_verify($pwd,$row[0])){
		if(!empty($name) && !empty($em) && !empty($ct)){
		         $sql0=<<<EOF
			update admin set name='$name',email='$em',contact='$ct' where email='$email';
EOF;
   	...
   	if(!$ret0) {
		...
		header('location:Admin_home.php');
				...
				$sql1=<<<EOF
			update admin set contact='$ct' where email='$email';
EOF;
   	...
   	if(!$ret1) {
		...
		header('location:Admin_home.php');
				...
				$sql2=<<<EOF
			update admin set email='$em' where email='$email';
EOF;
   	...
   	if(!$ret2) {
		...
		header('location:Admin_home.php');
				...
				$sql3=<<<EOF
			update admin set name='$name' where email='$email';
EOF;
   	...
   	if(!$ret3) {
		...
		header('location:Admin_home.php');
				...
				$sql4=<<<EOF
			update admin set email='$em',contact='$ct' where email='$email';
EOF;
	$ret4=pg_query($db,$sql4);
   	if(!$ret4) {
		...
		header('location:Admin_home.php');
				...
				$sql5=<<<EOF
			update admin set name='$name',contact='$ct' where email='$email';
EOF;
	$ret5=pg_query($db,$sql5);
   	if(!$ret5) {
		...
		header('location:Admin_home.php');
				...
				$sql6=<<<EOF
			update admin set name='$name',email='$em' where email='$email';
EOF;
